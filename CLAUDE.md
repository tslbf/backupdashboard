...
- Design tokens are `assetdashboard`'s `styles.css`, copied unchanged, with a
  backup-outcome section appended. **Don't drift the palette** — it went through
  several rounds with the owner on the other dashboard.
- Outcome colors are **status** colors: reserved, never reused to distinguish
  two servers. Every one ships with a glyph and a written label; status is never
  color alone.
- Charts carry **five** classes, not six: `running` and `unknown` fold into one
  slate "other" band (`chartCounts`). Two slates cannot be separated from each
  other, and both are transient states that don't belong in a trend. They stay
  fully distinct in chips and tables where a label carries the meaning.
- `missed` is violet, not another slate. It is the most dangerous state an
  estate has — nothing failed, nothing ran, nothing complained — and grey reads
  as "no data". Validated for CVD separation against the other four (worst
  adjacent pair ΔE 13.1 protan, 20.3 normal vision, all ≥ 3:1 on surface).
- No dual-axis charts. The duration card is two **small multiples** because the
  ...
  correlation.
- 2px gaps between stacked segments are surface showing through, not borders.
